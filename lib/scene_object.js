...
class Node{
    constructor(name, message_control=true){
        ...
        this.parent = null; // the parent will be rendered after all children
        // state
        ...
        if(message_control){
            this.subscribe(this.name, (node, topic, data) => {
                console.log(`${node.name} received message ${topic} with data ${data}`)
                if(topic == this.name){
                    if(data == "kill"){
                        this.remove_self();
                    }
    ...
    update(deltaT){
        ...
        this.accum_t += deltaT;
        this.delta_t = deltaT;
        this.update_self(deltaT); //implement in child class
        // update children
