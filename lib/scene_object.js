...
class SpriteNode extends Node{
    constructor(sp_handle, message_control=true){
        var name
        var sp_handle
        if(sp_handle instanceof SpriteHandle){
            sp_handle = [sp_handle]
            name = sp_handle.name
        }
        else if(sp_handle instanceof Array){
            if(sp_handle[0] instanceof SpriteHandle){
                throw `constructor: SpriteNode() expects 'SpriteHandle' or '[SprideHandel]', got ${image_handle}`
            }
            sp_handle = sp_handle
            name = sp_handle[0].name
        }
        super(name, message_control)
        this.sp_handle = sp_handle
        this.sp_index = 0;
        this.sp_cnt = this.sp_handle.lengt
        this.sp_w = 0
        this.sp_h = 0
        //[this.sp_w, this.sp_h] = [0,0]
        this.size = [this.acc_scale * this.sp_w, this.acc_scale * this.sp_h]
        this.forced_size = null
        this.mouseHovered = false;

        this.draw_self = function(){
            const sp = this.sp_handle[this.sp_index]
            console.log("draw")
            if(this.forced_size){
                sp.draw(0,0,this.forced_size[0]/this.acc_scale, this.forced_size[1]/this.acc_scale)
            }else{
                sp.draw()
            }
        }
    ...
    force_size(w,h){
        this.forced_size = [w,h]
    }
    unforce_size(){
        this.forced_size = null
    }
    mouse_hovered(){
        const s = this.forced_size? this.forced_size: this.size;
        const x_in = mouseX > this.acc_x && mouseX < this.acc_x + s[0];
        const y_in = mouseY > this.acc_y && mouseY < this.acc_y + s[1];
        this.mouseHovered = x_in && y_in;
        return this.mouseHovered
    }
    update(deltaT, dw,dh){
        super.update(deltaT, dw, dh);
        [this.sp_w, this.sp_h] = this.sp_handle[this.sp_index].get_size()
        this.size = [this.acc_scale * this.sp_w, this.acc_scale * this.sp_h]
    }
    next_sprite(){
        this.sp_index = (this.sp_index + 1) % this.sp_cnt
    }
    set_sprite(ind_or_name){
        if(typeof ind_or_name == "number"){
            this.sp_index = ind_or_name
        }
        else if(typeof ind_or_name == "string"){
            for(var i = 0; i < this.sp_cnt; i++){
                if(this.sp_handle[i].name == ind_or_name){
                    this.sp_index = i
                    break
                }
            }
        }else{
            throw `set_sprite: invalid argument ${ind_or_name}`
        }
    }
