...
class SpriteLoader{
    ...
    async load(onFinish){
        const filename = sprite_path.substring(this.sprite_path.lastIndexOf('/')+1);
        const scene_name = filename.substring(0, filename.lastIndexOf('.'));
        console.log("spriteLoader loading sprite: " + filename);
        // fetch json file 
        this.json = await fetch(sprite_path).then(response => response.json());
        // load image 
    ...
    check_sprite_available(sprite_name){
        ...
        if(this.json["frames"].hasAttribute(sprite_name)){
            return true;
        }
        else{
            console.error("" + sprite_name + " is not a valid sprite name");
            possible_names = Object.keys(this.json["frames"]);
